  ...
  _updateStatusLinks () {
    ...
    for (var i = 0; i < links.length; ++i) {
      ...
      if (mention) {
        ...
        // fedired: make remote usernames
        if (expandUsernames) {
          if (mention.get('acct') === mention.get('username')) {
            link.innerHTML = `@<span class="fedired-mention-local">${mention.get('username')}</span>`;
          } else {
            link.innerHTML = `@<span class="fedired-mention-remote">${mention.get('acct')}</span>`;
          }
        }
