...
describe("API", () => {
	test("success", async () => {
		...
		fetchMock.mockResponse(async (req) => {
			...
			if (req.method === "POST" && req.url === "https://fedired.test/api/i") {
				if (body.i === "TOKEN") {
	...
	test("with params", async () => {
		...
		fetchMock.mockResponse(async (req) => {
			...
			if (
				req.method === "POST" &&
				req.url === "https://fedired.test/api/notes/show"
			) {
		...
		expect(getFetchCall(fetchMock.mock.calls[0])).toEqual({
			url: "https://fedired.test/api/notes/show",
			method: "POST",
	...
	test("204 No Content で null が返る", async () => {
		...
		fetchMock.mockResponse(async (req) => {
			if (
				req.method === "POST" &&
				req.url === "https://fedired.test/api/reset-password"
			) {
				return { status: 204 };
			} else {
				return { status: 404 };
		...
		expect(getFetchCall(fetchMock.mock.calls[0])).toEqual({
			url: "https://fedired.test/api/reset-password",
			method: "POST",
